...
| `INSTALL_MODEL_ENDPOINT` | every profile | you | yes | Where the inference server answers. Inside your own network by default. |
| `INSTALL_MODEL_PROFILE` | every profile | wizard | yes | `local` for the inference server alone, `hosted` to allow an external provider. `local` is the default because a company that has not chosen to send text off its own hardware has not chosen it. |

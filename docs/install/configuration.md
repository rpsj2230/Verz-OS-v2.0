...
| `BRAIN_VAULT_ADDRESS` | every profile | you | none | Where this install's secrets vault answers, as an address the application container can reach. Empty, with the token below empty too, means this install runs no vault: the setup wizard and the console then keep no provider key and say so, and nothing stores one anywhere else. Set both or neither; one without the other is treated as no vault and the startup log names the missing one. Neither reaches the application from the base compose files: `docker-compose.vault.yml` hands both over and joins the application to the vault's network, it is composed in only where the vault runs, and the update and rollback scripts compose it themselves once this line holds a value. `ops/openbao/credential-slots.md` has the steps that go with it. |
| `BRAIN_VAULT_TOKEN` | every profile | you | none | The token the application presents to that vault, minted on your own vault against the `application` policy and never copied from another install. It lets the application put in and replace model provider keys and read when each was written, and nothing else in the vault. |

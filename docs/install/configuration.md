...
| `BRAIN_PRESIDIO_URL` | every profile | you | none | Where the personal data analyser answers. Leave it empty: on `standard` and `full` the product dials its own `presidio-analyzer` service on the internal `pii` network, and on `lite`, which runs no analyser, startup refuses a value, because the analyser is handed text before it is scrubbed and a value copied from another install would send that text to a host you did not choose. Set it only to move the analyser somewhere you run yourself. |
...
| `INSTALL_LARK_BASE` | every profile | you | yes | The token of the one Lark Base the Base knowledge use reads live, as it appears in the Base's link after `/base/`. Written by Connect Lark on the Connectors screen; `unset` means no Base is named and nothing is read. |
| `INSTALL_LARK_PLATFORM` | every profile | you | yes | Which Lark platform your tenant lives on: `larksuite.com` for Lark or `feishu.cn` for Feishu. It decides which host your Lark app's secret is sent to, so `unset` sends it nowhere. Written by Connect Lark. |
| `INSTALL_LARK_USES` | every profile | you | yes | What your Lark app is switched on for, comma-separated: `staff_list`, `knowledge_wiki`, `knowledge_base`, `chat_channel`, or `none`. Written by Connect Lark on the Connectors screen, only after the app's credential is kept in the vault. Nothing from Lark is copied: knowledge keeps a small index and reads content live. |

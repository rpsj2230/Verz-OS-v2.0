...
since 2026-09-21), plus what the host keeps for itself. `full` can also be started from one
file, `docker-compose.full.yml`, which is the generated merge of the files the installer names
and is held equal to them by test; the installer itself keeps naming the files. The cores and the disk are ratios rather than measurements, and they
...
| `furnish the install` | The application furnishes itself every time it starts, so on a healthy install this finds that done and is skipped. Otherwise it runs `python -m brain.ops.starter_store` in the application container, which writes the product's starter set once: one company-wide scope named `company`, the `starter` pack every joiner is given, and a row in the capability registry for every capability the product declares. It writes no person, no department and nothing of your company's, and it is what lets the first administrator grant anything at all, because a grant is written over a named scope. | Read the application's logs and run again. It is one transaction, so a failure leaves nothing half written, and a scope or pack you later retire is never put back. |

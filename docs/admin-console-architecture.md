...
| `data_transfer_routes`, `artifact_routes` | data transfer, export; artifacts | `admin:export` plus the audit screen's read; `read:artifact` | `ops.data_export` (0082 adds its form); trigger 0053 | audit trail is the only data set; the chain for a whole-ledger reader, the readable entries for anybody else |

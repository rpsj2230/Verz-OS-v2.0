...
retirement, under the subject `principal:<id>`, with the role, the covered principal, for an
acknowledgement its sha256 digest (the text stays on the row, the ledger holds no free text), and
`source` `role_grant`, so
...
DECLARE__DECLARATIONS__BEGIN
    ...
    IF NEW.acknowledgement IS NOT NULL AND v_action = 'grant' THEN
        v_details := v_details || jsonb_build_object(
            'acknowledged', 'separation_of_duties',
            'acknowledgement_digest',
            encode(sha256(convert_to(NEW.acknowledgement, 'UTF8')), 'hex')
        );
    END IF;

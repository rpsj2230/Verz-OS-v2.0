...
describe("what a failure says about itself on every registered page", () => {
  test.each(shard(2, 3))("%s says no reference came back when a proxy answered without one", async (pattern) => {
  ...
  test.each(shard(2, 3))("%s shows the API's sentence and a way to sign in again when a second factor is needed", async (pattern) => {

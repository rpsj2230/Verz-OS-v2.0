/**
 * A menu of actions opened from a button: arrow keys move, Enter chooses, Escape closes and puts
 * focus back on the button.
 *
 * Copied from shadcn/ui's Radix "vega" style at CLI 4.21.0 through the design spike. The focused
 * item is the accent's wash with the accent's text colour, measured together.
 *
 * **A destructive item never acts on its own.** `variant="destructive"` colours an item; choosing it
 * opens a confirmation, because `tests/destructive-confirmed.test.ts` holds that a destructive write
 * is reachable only through an element named `ConfirmAction`. A menu item that closes and opens a
 * dialog leaves nothing to return focus to, which is what `returnFocusTo` on the dialog's content is
 * for (see `focus-return.ts`).
 *
 * Task ids: M27.10.2
 */
...
const ITEM =
  "relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-inset:pl-8 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4";
...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        ITEM,
        "group/dropdown-menu-item data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-crit-wash data-[variant=destructive]:focus:text-destructive",
        className,
      )}
      {...props}
    />

...
GRANTS: Mapping[str, tuple[Grant, ...]] = {
    ...
    "u_elsewhere": tuple(
        ...
        for capability in EVERY_TAB_AND_PLANE
    ),
}
...
def manifest(template_id: str = "pricing_desk", *, summary: str = SUMMARY) -> TemplateManifest:
...
def install_rows(
    agent_id: str, *, overlay: Mapping[str, Any] | None = None, summary: str = SUMMARY
    ...
    signed = publish(manifest(summary=summary), key=KEY, signed_by="u_publisher", at=SIGNED_AT)
    instance = install(
        signed,
        key=KEY,
...
class StubSession(AsyncSession):
    """An `AsyncSession` that answers the three statements this router makes, and no other."""
...
def test_the_roster_lists_the_agents_the_callers_audience_covers(
    ...
    assert response.json()["items"] == [
        ...
        {"agent_id": "my_notes", "display_name": "My Notes"},
        {"agent_id": "web_helper", "display_name": "Web Helper"},
    ]
...
def test_no_roster_answer_carries_a_count_of_anything(client: TestClient, stored: Stored) -> None:
    ...
    assert set(body["items"][0]) == {"agent_id", "display_name"}
...
def test_the_install_of_an_agent_the_caller_may_not_see_is_never_read(
    ...
    assert [len(s.column_descriptions) for s in stored.statements] == [1]

    stored.statements.clear()
    assert workspace_of(client, "u_wide", "their_notes").status_code == 200
    assert [len(s.column_descriptions) for s in stored.statements] == [1, 2]
...
def test_the_owner_on_the_header_is_the_steward_and_never_the_builder(
    ...
    """`created_by` and `owner_id` differ on this row, and only the steward reaches the body.

    Delete this and a fallback puts the name of somebody who handed the agent on where the
    person who answers for it goes."""
    stored.agents["quote_helper"] = agent_row("quote_helper", owner_id="u_current")

    text = workspace_of(client, "u_narrow", "quote_helper").text

    assert '"owner_id":"u_current"' in text
    assert "u_builder" not in text

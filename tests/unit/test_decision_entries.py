...
def test_a_leash_change_appends_one_entry_per_moved_target_in_its_transaction() -> None:
    """A rung moved for one target in the effective leash is one entry naming both rungs, and a
    write that leaves the leash alone writes none. Delete this and loosening an agent from Shadow
    to Autonomous leaves no record of who did it."""
    from tests.fixtures.scratch_postgres import sql

    rung = json.dumps([{"target": "ticket", "scope": {"clauses": []}, "rung": 2}])
    ...
    held = [one for one in (before or []) if one.get("target") == "ticket"]
    was = AutonomyTier(max((one["rung"] for one in held), default=0)).name.lower()
    assert untouched == []
    [(actor, subject, details, trace)] = inside
    assert (actor, subject, trace) == ("u_actor", f"agent:{agent}", "trace-decision")
    assert details_of(details) == {"target": "ticket", "from_rung": was, "to_rung": "autonomous"}
    assert after == []

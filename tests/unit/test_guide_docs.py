...
def test_the_department_admin_fact_is_read_off_the_withheld_list() -> None:
    """Service levels and the five Install screens are the screens a department admin is not
    offered, and they are named here rather than read off the withheld list the fact is computed
    from. Delete this and the runbook fact can be computed from something else and agree with the
    registry by accident on every screen but those six."""
    offered = {
        one.key: dict(runbook_facts(one, registered_tools=()))["Offered to a department admin"]
        for one in SCREENS
    }

    for withheld in ("service_levels", "install", "updates", "recovery", "limits", "connections"):
        assert offered.pop(withheld) == "no", withheld

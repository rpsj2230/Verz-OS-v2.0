...
def test_the_auditable_action_set_is_closed_and_complete() -> None:
    ...
    `session_end` records a sign-in session ended before it lapsed, written by `0050`'s trigger
    ...
    `skill` records a skill added to the library, approved or rejected, written by `0056`'s

...
Every write runs `brain.tables.audit.attributed_to` for the caller first, from what
`brain.attribution` reads off the request, so `0102`'s trigger records who appointed or removed
whom.
...
class Attribution:
    """Who writes, at what reach, in which request: what `brain.attribution.of_request` reads."""

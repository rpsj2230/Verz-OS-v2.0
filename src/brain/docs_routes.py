...
def needs_rupash() -> HTMLResponse:
    ...
    **Paragraphs are joined before they are wrapped in a tag, and that is the whole fix.**
    ...
    return _markdown_page(path.read_text(encoding="utf-8"), "Needs Rupash")


#: The console audit `docs/admin-console.md` asks for, generated by the console's own tests.
CONSOLE_AUDIT_FILE = "console-audit.md"


@router.get("/build/console-audit", response_class=HTMLResponse, tags=["docs"])
def console_audit() -> HTMLResponse:
    """What an administrator would need to manage against what the console serves.

    Served beside the decisions page and rendered by the same code, because it is the same kind
    of document: markdown the repository carries, read by the owner in a browser. It is written
    by `console/tests/console-audit.test.ts` rather than by hand, and that test fails when the
    file differs from what the code says, so the page is as current as the commit it shipped in.
    A missing file is a 404 naming it, not an empty page: an audit that is absent must not read
    as an audit that found nothing.
    """
    path = DOCS / CONSOLE_AUDIT_FILE
    if not path.exists():
        return HTMLResponse(f"<h1>{CONSOLE_AUDIT_FILE} not published</h1>", status_code=404)
    return _markdown_page(path.read_text(encoding="utf-8"), "Console audit")


def _markdown_page(raw: str, title: str) -> HTMLResponse:
    """A markdown document from `docs`, rendered with the marks `_inline` knows.

    Shared by `needs_rupash` and `console_audit` rather than written twice, for the reason the
    first one's docstring gives: joining a paragraph before wrapping it is the fix, and a second
    renderer is a second place for a paragraph to be split per line again.
    """
    import html as _html

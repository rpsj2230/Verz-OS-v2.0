...
def check(url: str) -> int:
    ...
    # `libpq_conninfo` because every deployment writes `DATABASE_URL` in SQLAlchemy's form and
    ...
    with psycopg.connect(libpq_conninfo(url)) as conn, conn.cursor() as cur:
        cur.execute(

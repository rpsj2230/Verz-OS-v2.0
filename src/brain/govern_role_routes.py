...
the row, and `0102`'s trigger puts its digest on the ledger entry (M1.8.7). The warning is said
only to a caller who already holds the authority to make the appointment.
...
def _by(asked: Asking) -> Attribution:
    """The caller, their reach digest and the request trace, as `brain.attribution` reads them."""
    return Attribution(
